// Name: Perdue
// Description: FineSharp + DPX + HSLShift + PD80 Selective Color + Lightroom

// FineSharp (sstr=0.384, lstr=1.490)
float3 applyFineSharp(texture2d<float> tex, sampler smp, float2 uv, float2 texel, float3 center) {
    constexpr float sstr = 0.384;
    constexpr float lstr = 1.490;
...
// PD80 Selective Color v2
float3 applySelectiveColor(float3 color) {
    float cmax = max(color.r, max(color.g, color.b));
    float cmin = min(color.r, min(color.g, color.b));
    float delta = cmax - cmin;
    ...
    float redW = saturate(1.0 - abs(hue) * 8.0) + saturate(1.0 - abs(hue - 1.0) * 8.0);
    redW = min(redW, 1.0) * sat;
    float orangeW = saturate(1.0 - abs(hue - 0.08) * 8.0) * sat;
    float yellowW = saturate(1.0 - abs(hue - 0.16) * 8.0) * sat;

    float redLig = redW * 0.184;
    float redBla = redW * 0.116;
    float redYel = redW * -0.324;
    float orangeSat = orangeW * 0.088;
    float orangeLig = orangeW * 0.008;
    float yellowLig = yellowW * -0.204;

    float totalLig = redLig + orangeLig + yellowLig;
    rgb *= 1.0 + totalLig;
    rgb += redBla * 0.5;
    rgb.b -= redYel * 0.15;

    float luma = dot(rgb, float3(0.2126, 0.7152, 0.0722));
    rgb = mix(float3(luma), rgb, 1.0 + orangeSat);
...
fragment float4 fragment_main(VertexOut in [[stage_in]],
    ...
    color = applyFineSharp(tex, smp, uv, texel, color);
